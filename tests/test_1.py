...
def create_db():
    db_dir = Path("/tmp")
    target_db_name = "discard_test_db.sqlite"
    target_db_path = db_dir / target_db_name
    if target_db_path.exists():
        target_db_path.unlink()
    task_db = TaskDB(db_dir, name_override=target_db_name, autocreate=True)
    return [task_db, db_dir, target_db_name]

def test_tasks_1(create_db):
    task_db, db_dir, target_db_name = create_db

    with pytest.raises(Exception):
        bad_db = TaskDB('.', name_override="foooo")

    with pytest.raises(Exception):
        bad_db = TaskDB('/tmp_not_there')
    
    assert task_db.get_task_by_name('task1') is None
    assert task_db.get_task_by_id(1) is None
    assert task_db.get_tasks_by_status('ToDo') == []
    task1 = task_db.add_task('task1', 'foobar', 'ToDo')
    assert 'task1' in str(task1)
    task1.description = "Updated"
    assert task1.save()
    copy = task_db.get_task_by_name('task1')
    assert copy.task_id == 1
    assert copy == task1

    t2 = task_db.get_task_by_id(1)
    assert t2 is not None
    assert t2 == task1
    todos = task_db.get_tasks_by_status('ToDo')  
    assert len(todos) == 1
    t3 = todos[0]
    assert t3 == task1
    
    with pytest.raises(Exception):
        task2_bad = task_db.add_task('task1', 'foobar', 'ToDo')
    
    with pytest.raises(Exception):
        task2_bad = task_db.add_task('task2', 'foobar', 'blarch')
    
    with pytest.raises(Exception):
        task_db.get_tasks_by_status('foo')
    
    task2 = task_db.add_task('task2', None, 'ToDo')
    assert task2 != task1
    ...
    task_db2 = TaskDB(db_dir, name_override=target_db_name)

    todos = task_db2.get_tasks_by_status('ToDo')  
    assert len(todos) == 2
    t4 = todos[0]
    t5 = todos[1]
    assert t4 == task1
    ...
    tr_new  = TaskRecord(task_db=task_db2, task_id=None, name="new rec",
                         description="slarty", status='ToDo')
    tr_new.save()
    tr_renew = task_db2.get_task_by_id(tr_new.task_id)
...
def test_projects_1(create_db):
    ...
    proj_1 = task_db.add_project("proj_1", "some things")
    assert "proj_1" in str(proj_1)
    p1_copy = task_db.get_project_by_id(proj_1.project_id)
    assert proj_1.__dict__ == p1_copy.__dict__
    assert proj_1 == p1_copy
    assert proj_1.parent is None
    assert proj_1 == task_db.get_project_by_name(proj_1.name)
    assert task_db.get_project_by_name(proj_1.name + 'a') is None
    no_proj = task_db.get_project_by_id(-1)
    assert no_proj is None
    assert len(task_db.get_projects()) == 1
    assert len(task_db.get_projects_by_parent_id(None)) == 1
    
    # use the id form of parenting
    proj_2 = task_db.add_project("proj_2", "child of proj_1, some other things", parent_id=proj_1.project_id)
    # check __eq__ false
    ...
    assert proj_2.parent == task_db.get_project_by_name(proj_2.name).parent
    assert f"{proj_2.project_id}" in str(proj_2)
    assert task_db.get_project_by_id(proj_2.project_id).parent.project_id == proj_1.project_id
    assert len(task_db.get_projects()) == 2
    
    
    kids = proj_1.get_kids()
    assert proj_2 == kids[0]

    proj_3 = task_db.add_project("proj_3", None, parent=proj_1)
    
    # use the project form of parenting and save it from a new project object
    proj_4 = ProjectRecord(task_db, None, "proj_4", None, parent=proj_2)
    proj_4.save()
    assert proj_4.parent == proj_2
    with pytest.raises(Exception):
        proj_bad = ProjectRecord(task_db, None, "proj_5", None, parent=proj_2, parent_id=proj_1.project_id)
        
    
    # test cascade
    pid = proj_1.project_id
    proj_1.delete_from_db()
    no_proj = task_db.get_project_by_id(pid)
    assert no_proj is None
    no_proj = task_db.get_project_by_id(proj_2.project_id)
    assert no_proj is None
    no_proj = task_db.get_project_by_id(proj_3.project_id)
    assert no_proj is None
    ...
    proj_5 = ProjectRecord(task_db, None, "proj_5", None)
    proj_5.save()
    with pytest.raises(Exception):
        # dup name
        proj_6 = task_db.add_project("proj_5")
    proj_6 = ProjectRecord(task_db, None, "proj_6", None)
    proj_6.save()
    with pytest.raises(Exception):
        proj_6.name = "proj_5"
        proj_6.save()
    ...
    proj_6.save()
    ncopy = task_db.get_project_by_id(proj_6.project_id)
    ...
    with pytest.raises(Exception):
        # invalid parent_id
        proj_7 = task_db.add_project("proj_7", parent_id=-1)


def test_project_tasks(create_db):
    task_db, db_dir, target_db_name = create_db

    proj_1 = ProjectRecord(task_db=task_db, project_id=None, name="proj_1", description="some things")
    ...
    task2 = task_db.add_task('task2', 'blarch', 'ToDo')
    ...
    task1 = task_db.get_task_by_name('task1')
    orphans = task_db.get_project_by_name("Orphans")
    assert task1.project == orphans
    task2 = task_db.get_task_by_name('task2')
    assert task2.project == orphans

    # check that removing child project moves tasks up to parent project
    
    proj_2 = ProjectRecord(task_db=task_db, project_id=None, name="proj_2", description="some things")
    proj_2.save()
    proj_3 = ProjectRecord(task_db=task_db, project_id=None, name="proj_3", description="some more things",
                           parent=proj_2)
    proj_3.save()
    ...
    with pytest.raises(Exception):
        task_db.replace_task_project_refs(proj_2.project_id, -1)
...
def test_task_depends_1(create_db):
    ...
    task3 = task_db.add_task('task3', None, 'ToDo')
    task2.add_blocker(task3)
    assert len(task2.get_blockers()) == 2
    task4 = task_db.add_task('task4', None, 'ToDo')
    ...
    task5 = task_db.add_task('task5', None, 'Done')
    ...
    task5 = task_db.get_task_by_id(task5.task_id)
...
def test_phases_1(create_db):
    ...
    proj_1 = task_db.add_project("proj_1", "some things")
    phase_1 = task_db.add_phase("phase_1", "phase of project 1 some things", project=proj_1)
    assert "phase_1" in str(phase_1)
    p1_copy = task_db.get_phase_by_id(phase_1.phase_id)
    assert phase_1 == p1_copy
    assert phase_1.__dict__ == p1_copy.__dict__
    assert phase_1.project == proj_1
    p1_copy_2 = task_db.get_phase_by_name(phase_1.name)
    assert phase_1 == p1_copy_2
    no_phase = task_db.get_phase_by_id(-1)
    assert no_phase is None
    no_phase = task_db.get_phase_by_name('foobaar')
    assert no_phase is None
    assert len(task_db.get_phases_by_project_id(proj_1.project_id)) == 1
    with pytest.raises(Exception):
        # used name
        task_db.add_phase("phase_1", "phase of project 1 some things", project=proj_1)
    
    with pytest.raises(Exception):
        # used name
        bogus = PhaseRecord(task_db, None, "phase_1", description=None, project_id=proj_1.project_id)
        bogus.save()
    
    with pytest.raises(Exception):
        # no project
        task_db.add_phase("phase_x", "phase of project 1 some things", project=None)
    
    with pytest.raises(Exception):
        # bad project id
        task_db.add_phase("phase_x", "phase of project 1 some things", project_id=99999999)
    
    with pytest.raises(Exception):
        # bad phase_id 
        bogus = PhaseRecord(task_db, 999999, "phase_10", description=None, project_id=proj_1.project_id)
        bogus.save()
    ...
    assert task_db.get_phase_by_id(phase_2.phase_id).project == proj_1
    assert len(task_db.get_phases_by_project_id(proj_1.project_id)) == 2
    assert phase_2.follows == phase_1

    # just make sure it
    phase_2.name = "foo"
    phase_2.save()
    assert task_db.get_phase_by_id(phase_2.phase_id).name == "foo"
    
    
    with pytest.raises(Exception):
        # used name and differrent id
        bogus = PhaseRecord(task_db, phase_2.id, phase_1.name, description=None, project_id=proj_1.project_id)
        bogus.save()
    
    
    phase_3 = PhaseRecord(task_db, None, "phase_3", description=None, project_id=proj_1.project_id)
    ...
    assert task_db.get_phase_by_id(orig_p3_id) is None
...
def test_phases_links_1(create_db):
    ...
    proj_1 = task_db.add_project("proj_1", "some things")
    with pytest.raises(Exception):
        task_db.save_phase("phase_bad", description=None, project_id=proj_1.project_id,
                                pfollows_id=9999999)
    ...
    assert task_db.get_phase_that_follows(999999) is None
    with pytest.raises(Exception):
        phase_1.save()
    phase_1.follow_id = None
    phase_2 = proj_1.new_phase("phase_2", description=None, follows=phase_1)
    assert task_db.get_phase_by_id(phase_2.phase_id).follows == phase_1
    ...
    assert task_db.get_phase_by_id(phase_3.phase_id).follows == phase_2
    ...
    phase_5 = PhaseRecord(task_db, None, "phase_5", description=None, project_id=proj_1.project_id)
    phase_5.save()
    phase_4 = PhaseRecord(task_db, None, "phase_4", description=None, project_id=proj_1.project_id)
    phase_4.save()

    phase_4.follows = phase_3
    phase_4.save()
    assert task_db.get_phase_by_id(phase_4.phase_id).follows == phase_3
    phase_5.follows = phase_4
    phase_5.save()
    assert task_db.get_phase_by_id(phase_5.phase_id).follows == phase_4
    ...
    proj_2 = task_db.add_project("proj_2", "other things")
    phase_5.project = proj_2
    with pytest.raises(Exception):
        phase_5.save()
    # restore it, which also checks for reqression on double follow test on re-save
    ...
    phase_5.save()
    assert task_db.get_phase_by_id(phase_5.phase_id).follows == phase_2
    assert task_db.get_phase_by_id(phase_3.phase_id).follows == phase_5
    ...
    phase_5_new = task_db.get_phase_by_id(phase_5.phase_id)
...
def test_phase_tasks(create_db):
    ...
    phase_1 = PhaseRecord(task_db, None, "phase_1", description=None, project_id=proj_1.project_id)
    ...
    with pytest.raises(Exception):
        task_db.replace_task_phase_refs(phase_1.phase_id, -1)
    ...
    proj_2 = ProjectRecord(task_db=task_db, project_id=None, name="proj_2", description="some things")
    proj_2.save()
    phase_2 = PhaseRecord(task_db, None, "phase_2", description=None, project_id=proj_2.project_id)
    phase_2.save()

    task3 = task_db.add_task('task3', 'bebebeb', 'ToDo', project_id=proj_2.project_id,
                             phase_id=phase_2.phase_id)
    assert len(phase_2.get_tasks()) == 1

    proj_3 = ProjectRecord(task_db=task_db, project_id=None, name="proj_3", description="some things")
    proj_3.save()
    phase_3 = PhaseRecord(task_db, None, "phase_3", description=None, project_id=proj_3.project_id)
    phase_3.save()
    ...
    task_db.replace_task_phase_refs(phase_3.phase_id, phase_2.phase_id)
    assert len(phase_2.get_tasks()) == 1
    ...
    task_db.replace_task_phase_refs(phase_2.phase_id, phase_2.phase_id)
    assert len(phase_2.get_tasks()) == 1
    assert len(phase_3.get_tasks()) == 0
    assert len(proj_2.get_tasks()) == 1
    assert len(proj_3.get_tasks()) == 0

    proj_4 = ProjectRecord(task_db=task_db, project_id=None, name="proj_4", description=None)
    proj_4.save()
    phase_4 = PhaseRecord(task_db, None, "phase_4", description=None, project_id=proj_4.project_id)
    phase_4.save()
    assert len(phase_4.get_tasks()) == 0
    task4 = task_db.add_task('task4', 'bebebeb', 'ToDo', project_id=proj_4.project_id,
                             phase_id=phase_4.phase_id)
    assert len(phase_4.get_tasks()) == 1
    assert len(proj_4.get_tasks()) == 1

    proj_5 = ProjectRecord(task_db=task_db, project_id=None, name="proj_5", description=None,
                           parent_id=proj_4.project_id)
    proj_5.save()
    phase_5 = PhaseRecord(task_db, None, "phase_5", description=None, project_id=proj_5.project_id)
    phase_5.save()
    phase_6 = PhaseRecord(task_db, None, "phase_6", description=None, project_id=proj_5.project_id)
    phase_6.save()
    ...
    task4 = task_db.get_task_by_id(task4.task_id)
    ...
    phase_4 = task_db.get_phase_by_id(phase_4.phase_id)
...
def test_wrappers_1(create_db):
    ...
    # for some reason just trying to access the _wrapped attr directly
    # fails to get recorded by coverage, but calling __getattr__ does. Hmph
    wrapped = proj_wrap.__getattr__('_wrapped')
    proj_wrap._wrapped = wrapped

    phase_1 = task_db.add_phase('phase_1', '', project=proj_1)
    ...
    task_1 = task_db.add_task('task1', 'foo', 'ToDo', project_id=proj_1.project_id, phase_id=phase_1.phase_id)
    ...
    proj_2 = task_db.add_project("proj_2", "other things")
    phase_2 = task_db.add_phase('phase_2', '', project=proj_1)
    ...
    task_2 = task_db.add_task('task2', 'foo', 'ToDo', project_id=proj_1.project_id,
                              phase_id=phase_1.phase_id)
    phase_3 = task_db.add_phase('phase_3', '', project=proj_2)
    task_2_wrap = task_2.get_tracking_wrapper()
    task_2.project = proj_2
    assert task_2_wrap.is_changed()
    task_2_wrap.save()
    task_2 = task_db.get_task_by_id(task_2.task_id)
...
def test_json(create_db):
    ...
    task_1 = task_db.add_task('task1', 'foo', 'ToDo', project_id=proj_1.project_id)
    # now just make sure our filter works with supplied value
    j1 = json.dumps(task_1.to_json_dict())
    fw2 = FilterWrapper(task_1.__dict__)
...
def test_backups(create_db):
    ...
    # a bunch of records
    o_proj_1 = task_db.add_project("proj_1", "some things")
    o_proj_2 = task_db.add_project("proj_2", "some things", parent=o_proj_1)
    o_phase_1 = task_db.add_phase('phase_1', '', project=o_proj_1)
    o_task_1 = task_db.add_task('task1', 'foo', 'ToDo', project_id=o_proj_1.project_id,
                                phase_id=o_phase_1.phase_id)
    o_task_2 = task_db.add_task('task2', 'foo', 'ToDo', project_id=o_proj_1.project_id,
                                phase_id=o_phase_1.phase_id)

    o_task_3 = task_db.add_task('task3', 'foo', 'ToDo', project_id=o_proj_1.project_id)
    ...
    target_db_path = Path(db_dir, 'test_backup.sqlite')
    if target_db_path.exists():
        target_db_path.unlink()
    task_db.make_backup(db_dir, 'test_backup.sqlite')
    bdb = TaskDB(db_dir, 'test_backup.sqlite')

    n_proj_1 = bdb.get_project_by_name('proj_1')
    # should be a diffent id, so equal fail
